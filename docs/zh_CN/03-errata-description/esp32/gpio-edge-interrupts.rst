[GPIO-3.14] 同一组 GPIO 管脚内，边沿中断不能和其他中断一起使用
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
在中断配置寄存器上，GPIO0 ~ GPIO31 共用一组寄存器，GPIO32 ~ GPIO39 共用一组寄存器，RTC GPIO0 ~ RTC GPIO17 共用另一组寄存器。在同一组内，如果某一管脚被配置为边沿中断，那么其他中断（包括边沿中断和电平中断）将不可用。
